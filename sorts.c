...
void char_print_system(){
    ...
    while(1){
        printf("\nDo you want to repeat the process? [Y/y for yes (uses recursion) | N/n for no (terminates the program)]:");
        scanf(" %c", &ch);
        switch(ch){
            case 'Y':
            case 'y': char_print_system();
            break;
            case 'N':
            case 'n': exit(0);
            break;
